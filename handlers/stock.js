...
function stock(channel, message, params) {
    ...
    message.client.users.forEach(member => {
        if (!member.bot) {
            getStock(tableSvc, member.username, function (error, result, response) {
    ...
    if (splitParams[0] === 'balance') {
        getStock(tableSvc, message.author.username, function (error, result, response) {
            ...
            let balanceString = `${message.author.username}\'s balance is ${result}$$`;
                ...
                getStock(tableSvc, recipient, function (error, receiverStock, response) {
                    if (typeof receiverStock === 'number') {
                        getStock(tableSvc, message.author.username, function (error, senderStock, response) {
                            if ((senderStock - parseInt(sendAmount)) > 0 && typeof senderStock === 'number') {

                                const newSenderTotal = senderStock - parseInt(sendAmount);
                                const newReceiverTotal = receiverStock + parseInt(sendAmount);
    ...
    tableSvc.queryEntities(TABLENAME, query, null, function (error, result, response) {
        if (!error) {
            ...
            if (result.entries.length > 0) {
                callback(error, parseInt(result.entries[0].schreefstock._), response);
            }
            else {
